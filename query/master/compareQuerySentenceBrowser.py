...
def runScript():
	...
	for l in PTfile:
		...
		if "Pi" in pi:
			...
			index=open(piaddress+domain+".doc.topics.index")
		else:
			piaddress="http://"+pi+"/"+domain+"/"
			index=urllib.urlopen(piaddress+domain+".doc.topics.index")
	...
	for doc in docs:					
		...
		for dline in peardoc:
			...
			if m3:
				...
				pagescore+=scoreWO
				if numSentences > 50:			#For speed (and accuracy!), only process first 50 sentences of the file
					break
		if numSentences > 0:
