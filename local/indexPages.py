...
def loadURLs(pear):
	...
	path_to_dict=path_to_PeARS+pear+"/urls.dict.txt"
...
def lynx(pages_file,pear):
	...
	for url in pages:
		...
		if url not in url_dict:		#Don't lynx again a page that is available already on that pear
			...
			t.close()
			file_id=str(c)+".txt"
			lynxfile = open (path_to_PeARS+pear+"/originals/"+file_id,'w')
			...
			lynxtmp = open (path_to_PeARS+"lynx.tmp",'r')
			...
			lynxfile.close()
			os.remove(path_to_PeARS+"lynx.tmp")
			c+=1
...
def postag(pear):
	...
	for file_id in new_files_ids:
		lynx_file=path_to_PeARS+pear+"/originals/"+file_id
		lemmatised_file=path_to_PeARS+pear+"/lemmas/"+file_id
...
def update_shared_ids(pear):
	dist=""
	pear_profile=open(path_to_PeARS+pear+"/profile.txt","r")
	...
	pears_dists=[]	
	pears_ids=open(path_to_PeARS+"shared_pears_ids.txt",'r')
	for l in pears_ids:	
		l=l.rstrip('\n')
		m=re.search(pear,l)
		if not m:
	...
	pears_ids=open(path_to_PeARS+"shared_pears_ids.txt",'w')
...
def runScript(a1,a2):
	...
	if not os.path.isdir(path_to_PeARS+pear):
		os.makedirs(path_to_PeARS+pear)
	if not os.path.exists(path_to_PeARS+"shared_pears_ids.txt"):
		open(path_to_PeARS+"shared_pears_ids.txt",'w').close()
	if not os.path.isdir(path_to_PeARS+pear+"/originals"):
		os.makedirs(path_to_PeARS+pear+"/originals")
	if not os.path.isdir(path_to_PeARS+pear+"/lemmas"):
		os.makedirs(path_to_PeARS+pear+"/lemmas")
	if not os.path.isdir(path_to_PeARS+pear+"/distsem"):
